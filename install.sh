...
echo 'xhost +local:root && docker run -it --rm --shm-size=4g --workdir=/root --hostname=androcker --net=host --privileged -e DISPLAY -e GENY=$1 -e AHOME=$HOME -v /var/run/docker.sock:/var/run/docker.sock -v $HOME/.androcker:/root scarfaced/androcker:androcker /bin/bash && xhost -local:root' > $HOME/.local/bin/androcker && \
